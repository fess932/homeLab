...
- Поиск Tuya в сети: запрос приложения на UDP 7000, приём анонсов на UDP 6666/6667/7000 и проверка TCP 6668 в подсети (по умолчанию — /24 вокруг адресов HomeDeck, не более 1024 адресов). В Docker с bridge-сетью широковещательные анонсы не доходят: для поиска с id и версией нужен `network_mode: host`, иначе находится только открытый порт. В Windows входящий UDP может закрыть брандмауэр.
