...
Образ `ghcr.io/fess932/homelab` публикует CI (`.github/workflows/ci.yml`): `latest` — последний `main`, `sha-<коммит>` — конкретная сборка, `vX.Y.Z` — git-тег `vX.Y.Z`. Платформы — linux/amd64 и linux/arm64. Если пакет в GitHub приватный, перед `pull` нужен `docker login ghcr.io` с токеном, у которого есть право `read:packages`.
...
| `HOMEDECK_RETENTION` | `50y` | Хранение метрик: число и суффикс `h`, `d`, `w`, `y`, не больше `100y` (предел VictoriaMetrics); по умолчанию история фактически не удаляется |
...
Внутренние адреса `HOMEDECK_VM_LISTEN` (`127.0.0.1:8428`), `HOMEDECK_INTERNAL_LISTEN` (`127.0.0.1:9091`), `HOMEDECK_EGRESS_LISTEN` (`127.0.0.1:9092`) слушают только loopback. `compose.yaml` по умолчанию запускает контейнер в сети хоста (нужна поиску устройств), поэтому эти порты занимаются на loopback хоста: если там уже есть что-то на 8428, 9091 или 9092, задайте другие адреса. Снаружи доступен только `HOMEDECK_LISTEN` (8080). Без поиска устройств контейнер может работать и в bridge-сети с `ports: ["8080:8080"]`.
...
- Применение: `desired_revision` растёт при изменении источников и секретов → dry-run `victoria-metrics -promscrape.config.dryRun -promscrape.config.strictParse` → атомарная замена файлов → `POST /-/reload`, если конфиг изменился (в Windows нет SIGHUP, поэтому VictoriaMetrics запускается с `-promscrape.configCheckInterval=2s` и сама перечитывает изменившийся файл; тот же конфиг не перезагружается) → проверка `vm_promscrape_config_reloads_errors_total` и `vm_promscrape_config_last_reload_success_timestamp_seconds` → `applied_revision`. При ошибке остаётся прежний рабочий конфиг, ошибка видна в UI и `/api/v1/status`, reconciler повторяет попытку каждые 30 с. Последние 20 применённых конфигов сохраняются в `/data/config-revisions/scrape-*.yaml`.
...
- Устройства со своим API опрашивает сам HomeDeck (компонент `devices`), по циклу на устройство с интервалом и таймаутом из его настроек. Как говорить с устройством, решает драйвер из `drivers/<тип>`; настройки драйвера хранятся в поле `config` устройства и проверяются драйвером. Значения публикуются на внутреннем `127.0.0.1:9091/metrics` и попадают в VictoriaMetrics с системным источником `homedeck`.
...
- Поиск Tuya в сети: запрос приложения на UDP 7000, приём анонсов на UDP 6666/6667/7000 и проверка TCP 6668 в подсети (по умолчанию — /24 вокруг адресов HomeDeck, не более 1024 адресов). Для анонсов в Docker нужна сеть хоста — она включена в `compose.yaml` по умолчанию; в bridge-сети находится только открытый порт, без id и версии. В Windows входящий UDP может закрыть брандмауэр.
- Аккаунт Smart Life подключается входом по QR (код пользователя из приложения + сканирование). Токены аккаунта хранятся зашифрованным секретом типа `account:tuya` и обновляются сами. Из аккаунта при добавлении берутся ключ и описание точек данных; ключ сервер получает сам и в браузер не отдаёт. Облако используется только при подключении и добавлении, опрос — локальный. Вход идёт через API интеграции Home Assistant с её публичным идентификатором клиента: это неофициальное использование, Tuya может его ограничить.
- HTTP JSON: GET по URL, значения по путям через точку (`meters.0.power`), ответ до 1 MiB, множитель для перевода единиц.
- Опрос и поиск идут через ту же политику исходящих соединений, что проверки: устройство в loopback или link-local недоступно.
- Экспорт содержит устройства без ключей и аккаунтов; при импорте устройство без найденного секрета сохраняется выключенным.
...
2. `docker compose pull && docker compose up -d`. Чтобы обновления были предсказуемыми, в `compose.yaml` можно указать фиксированный тег (`sha-<коммит>`, `vX.Y.Z`) и менять его вручную.
...
Откат на старый образ после миграции невозможен без копии: старая версия откажется стартовать с сообщением «схема базы новее поддерживаемой». Восстановите копию `/data`, сделанную до обновления, и укажите в `compose.yaml` прежний тег.
...
- Загрузка файлов: PNG, JPEG, WebP до 5 MiB и 8192×8192, тип проверяется по содержимому; SVG до 256 KiB без скриптов, обработчиков событий и встроенного HTML. Иконки сайтов сервер скачивает сам при сохранении сервиса или ссылки (до 1 MiB, до 5 редиректов, без loopback и адресов метаданных). Файлы отдаются с `Content-Security-Policy: sandbox` и `nosniff`.
