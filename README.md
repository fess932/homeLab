...
**Нормы значений.** Числа и графики устройств окрашиваются по нормам: жёлтый — пора обратить внимание, красный — плохо. Нормы задаются в коде, в интерфейсе их нет:

- если устройство само хранит пороги тревоги (у датчиков Tuya это `maxtemp_set`/`minitemp_set`, `maxhum_set`/`minihum_set`, задаются в приложении), берутся они;
- иначе — типовые нормы из `drivers/thresholds.go`: влажность 30–60 % (ниже 20 и выше 70 — красный), CO₂ 1000/1500 ppm, PM2.5 35/75, PM10 50/100 мкг/м³, формальдегид 0,08/0,1 мг/м³, заряд батареи ниже 20/10 %. Температуры там нет: датчик может стоять и дома, и на улице;
- для PM2.5 и PM10 заданы ещё суточные нормы ВОЗ (15 и 45 мкг/м³). Сервер считает среднее за последние 24 часа; если оно выше нормы, под значением появляется строка «за сутки 18 мкг/м³ — выше нормы 15». Пока данных меньше суток или среднее в норме, строки нет.

Поменять нормы — поправить `drivers/thresholds.go`: у порога `Below` — «ниже значения», `Window: "24h"` — норма для среднего за сутки.
