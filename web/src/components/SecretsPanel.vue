...
function open(s?: Secret) {
  ...
  editing.value = {
    id: s?.id ?? null,
    input: { name: s?.name ?? '', kind: s?.kind ?? 'basic', username: '', password: '', token: '' },
  }
}
  ...
  <section class="card">
    ...
    <ul class="secrets">
      <li v-for="s in secrets" :key="s.id">
        ...
        <span class="small muted">{{ s.used_by.length ? t('sources.usedBy', { list: s.used_by.join(', ') }) : t('sources.notUsed') }}</span>
        <span class="spacer" />
